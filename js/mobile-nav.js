...
window.addEventListener("mouseup", function (event) {
    if (event.target !== nav && event.target.parentNode !== nav && event.target.parentNode.parentNode !== nav && event.target.parentNode.parentNode.parentNode !== nav && event.target !== navToggleSwitch && event.target.parentNode !== navToggleSwitch) {
        nav.className = "hidden";
        navToggleSwitch.className = "normal";
    }
});
